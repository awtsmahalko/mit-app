...
if(isset($data->ip_id) && !empty($data->ip_id) ){
    ...
    $fetch_item_packaging = $mysqli_connect->query("SELECT item_id, packaging_id from tbl_items_packaging where ip_id='$ip_id' ") or die(mysqli_error());
    ...
    if($ip_id > 0 AND $item_id > 0 AND $packaging_id > 0){
        ...
        $fetch_rows = $mysqli_connect->query("SELECT count(release_id), release_detail_id from tbl_release_details where release_id='$release_id' AND item_id = '$item_id' AND packaging_id = '$packaging_id'") or die(mysqli_error());

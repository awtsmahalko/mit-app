...
if (isset($_POST['hidden_id'])) {
	...
	if ($module == 'add') {
		$fetch_rows = $mysqli_connect->query("SELECT count(item_id) from tbl_items where item_name='$item_name'") or die($mysqli_connect->error);
		...
		$fetch_count_rows = $mysqli_connect->query("SELECT count(item_id) from tbl_items where item_name='$item_name' and item_id != '$item_id' ") or die($mysqli_connect->error);
		$count_rows = $fetch_count_rows->fetch_array();

		if ($count_rows[0] > 0) {
			echo 2;
		} else {
			$sql = $mysqli_connect->query("UPDATE `tbl_items` SET `item_name`='$item_name',`item_desc`='$item_desc',`item_serial_no`='$item_serial_no',`date_modified`='$date' WHERE item_id='$item_id'") or die($mysqli_connect->error);

...
$fetch_details = $mysqli_connect->query("SELECT pr.* from tbl_purchase_request_details AS pr,tbl_items AS i WHERE i.item_id = pr.item_id AND pr_id = '$pr_id' ORDER BY item_name ASC") or die($mysqli_connect->error);
...
<table width="100%" style="font-size:10pt;border-collapse: collapse;font-family:'Arial'">
    ...
    <tr>
        <td colspan='4' style="font-weight:bold;font-family:'Arial Bold'">OFFICE OF BIDS AND AWARDS COMMITTEE</td>
        <td colspan='2'>Fund: </td>
        <td colspan='4' style="border-bottom:1px solid;"></td>
    </tr>
    <tr align="center">
        <td colspan='10' style="font-size: 14pt;font-weight:bold;font-family:'Baskerville Old Face'">REQUEST FOR QUOTATION</td>
    </tr>
    <tr>
    ...
    while ($row_det = $fetch_details->fetch_array()) {
        echo '<tr align="center">
                    ...
                    <td style="border:1px solid;text-align:left;" colspan="5">' . getData("item_name", "tbl_items", "item_id", $row_det['item_id']) . '</td>
                    <td style="border:1px solid;">' . $row_det['qty'] . '</td>
                    <td style="border:1px solid;">' . getData("packaging_name", "tbl_packaging", "packaging_id", $row_det['packaging_id']) . '</td>
